...
Cost in this model concentrates entirely on mass production. Reef coverage scales linearly with the number of sensors produced. The naturalness argument holds only if the physical interaction between sensors and reef (repeated sinking, rising, occasional snagging) is benign — which is why early testing targets flat, sandy zones before approaching dense reef.
...
In Phase 1, sensors float passively after surfacing and are recovered by boat using GPS tracking. In Phase 2, the sensor is motorized for the return trip: it aims at the shore (a wide target), at constant speed, in a straight line. In both configurations, the loss rate is expected to be low: the sensor is held by the cable during capture and ascent, and either collected by boat or propelled to shore during transit. Losses would come from retrieval failure, motor failure, GPS loss, or extreme weather. This is a non-zero but bounded risk.
...
The simulation and coverage work in Part II validates the passive envelope: if cable forces, terrain coverage, or anchor loads don't hold, nothing else matters. The components that will decide the project — spool, wet-mate connector, energy budget — still need physical validation (see the rationale in Part II's opening note).
