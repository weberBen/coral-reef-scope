...
ReefScope inverts the maintenance problem. Instead of sending divers to service fixed equipment, the equipment services itself. Whether this inversion actually works depends on how many cycles it survives, how much each cycle costs, and how much breaks along the way. The answer comes from testing, not from design documents.
...
**Phase 3: Surface transit and data transmission.** Once at the surface, the sensor separates from the buoy and heads for shore. The cable stays attached to its own buoy, supported by intermediate floats along its length; it only needs to hold its own weight now. The sensor takes part of the buoy's flotation with it to ensure it floats for the trip, and heads for shore as a simple motorized float: straight line, constant speed, aimed at the coast (which is a wide target). Not a drone, not a submarine. No 3D navigation, no obstacle avoidance, no underwater intelligence. The energy for this transit is budgeted from the sensor's battery (the previous cycle's sensor) or from the fresh sensor's charge. Captured data is transmitted by radio to a shore station. The transmission delay is approximately 24 hours from capture to reception.
...
The system is designed to be deployed incrementally. Each phase exists to test assumptions from the previous one, not to deliver a finished product. Phase 2 only makes sense if Phase 1 measurements confirm the cycle closes. The sensor swarm direction only makes sense if Phases 1 and 2 demonstrate that the monitoring model works at all.
...
Cost in this model concentrates entirely on mass production. Reef coverage scales linearly with the number of sensors produced. The naturalité argument holds only if the physical interaction between sensors and reef (repeated sinking, rising, occasional snagging) is benign — which is why early testing targets flat, sandy zones before approaching dense reef.

The slow-descent approach draws from [TumblerBots (Romanello et al. 2024)](https://arxiv.org/abs/2410.23049): a drone-deployable tumbling capsule that descends at 0.8–2.5 m/s, detaches a sensing pod, and recovers by chemical buoyancy inflation — no battery needed, ecologically safe materials. The free-deployment model draws from [Ramsby et al. 2026](https://onlinelibrary.wiley.com/doi/10.1111/rec.70206), who dropped coral recruitment devices from the surface with 62–89% retention rates and no substrate damage on landing. Both demonstrate that free-deployed hardware can reach the seabed without mechanical anchoring or diver intervention (see also the implications for recovery in [What comes after cables](#what-comes-after-cables)).
...
The short-term track is trial and error on real sites, cycling between field tests and simulation adjustments as fast as possible. The real-world forces in calm to moderate conditions (the typical operating envelope for reef monitoring) are well-understood and the engineering is not extreme. A 10mm cable at 15m depth under 0.3 m/s current is a tractable problem. Building a few prototypes, deploying them, measuring what actually happens, feeding the results back into the simulation, adjusting, and deploying again will produce usable engineering data faster than waiting for a perfect simulation.
...
The tool computes geometric coverage: what is visible from each camera position given the 3D shape of the reef, accounting for terrain occlusion but assuming clear water. This is not naive optimism. Even in perfectly clear water, the complex geometry of a reef (overhangs, crevices, ridges) means that no set of camera positions covers everything. Photogrammetric reconstruction from ascent depends on what you can see from different angles at different altitudes, and geometric occlusion is the binding constraint before turbidity even enters the picture. Knowing this ceiling (the best you could possibly do) is the necessary first step. Turbidity reduces coverage further, but it does not invalidate the geometric analysis; it layers on top of it.
...
There is no deployment phase that avoids these components. The spool and wet-mate connector are required from the very first station deployed in Phase 1. There is no cheap validation path that sidesteps them. The first physical test already requires the hardest component to work.

This is why physical validation comes before simulation, not the other way around. The three measurements that collapse the most uncertainty at once are: rewind energy plus round-trip transit energy at a realistic offshore distance (do the 24 hours survive?), loss rate per cycle on a candidate site (does the OPEX argument hold?), and a bench-tested wet-mate docking connection with accelerated biofouling (does the cycle close at all?). Everything else in this document is downstream of those three numbers. The first test is one station, one cycle, one site.

The deliberate design choice is to concentrate fragility into very few components (the spool, the connector) rather than distributing small failure points across the whole system. Standard resilience thinking distributes risk so that no single failure is catastrophic. Here we do the opposite: if the spool fails, the station fails completely. But because the spool and connector have a single, well-defined job, they can be engineered with extreme robustness and multiple fallback mechanisms in isolation. It is easier to harden one component whose only task is "unwind on power loss, rewind on command, survive immersion" than to harden a complex distributed system where failure can emerge from any interaction.
...
In Phases 1 and 2, the sensor is motorized for the return trip. It has GPS, a motor, and energy budgeted for transit. It aims at the shore, which is a wide target. The loss rate in this configuration is expected to be low: the sensor is held by the cable during capture and ascent, and propelled during transit. Losses would come from motor failure, GPS loss, or extreme weather. This is a non-zero but bounded risk.

In the hypothetical passive drift mode (future, no motor), the sensor simply floats and the current carries it. The loss rate will be higher. This is an accepted trade-off: by the time passive drift is viable, sensor unit cost should be low enough to absorb losses, and autonomous surface collectors (simple boats with nets) should exist to recover drifters periodically.

In both cases, each loss is 24 hours of data and one unit of hardware. The loss rate conditions OPEX directly and needs to be quantified during Phase 1 trials with real hardware in real water. We expect the answer depends heavily on site selection (sheltered lagoon vs. exposed outer reef).
...
This concept becomes relevant only when sensor unit cost drops far enough that loss rates and simplified logistics outweigh the complexity of per-unit cable-based recovery. One path toward acceptably cheap, acceptably losable sensors is transient electronics: devices engineered to degrade safely in the environment after their mission. Foundry-fabricated CMOS on dissolvable silicon-on-insulator wafers already demonstrates functional logic gates ([Bae et al. 2025](https://www.beilstein-journals.org/bjnano/articles/16/109)), and AI-driven materials discovery is accelerating the search for biodegradable metals, polymers, and semiconductors suited to marine conditions ([Motadayen et al. 2024](https://accscience.com/journal/IJAMD/1/2/10.36922/ijamd.3173)). Transient-level microprocessors do not exist yet, but the trajectory points there. Phases 1 and 2 are where the engineering is focused now.
...
The simulation and coverage work presented in Part II addresses the passive, well-understood parts of the system. The components that will decide the project (spool, wet-mate connector, energy budget) have no simulation and no validation yet. This is a fair criticism, and the document does not hide it. But it is not an accident or a misallocation of effort. This work was built in a few days, with no access to marine hardware or a deployment site. The spool, the connector, and the energy budget cannot be meaningfully simulated before field measurements exist, because they depend on too many unknown parameters (materials, form factor, fouling rates, sea state at the deployment site). The cable simulation and the coverage tool address problems that were already non-trivial to set up with existing tools, despite being the better-understood part of the system. If the passive envelope does not hold (cable fails, coverage is geometrically impossible, forces exceed anchor capacity), nothing else matters. Verifying the scaffold first, even when the scaffold is not the hardest part, is not wasted work. It is the part that could be done now, and it provides the first sanity check on whether the overall concept is viable before committing to the harder, more expensive physical validation. For example, the tension on a taut cable at 5-10 meters depth under swell is not a number you can estimate by intuition. You need a simulation to get an order of magnitude, and that order of magnitude is what orients the rest of the engineering: what cable diameter, what material, what anchor capacity, what safety margin. Without it, every downstream choice is a guess.
